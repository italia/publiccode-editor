const yamlMimeTypes = ["application/yaml", "application/x-yaml", "text/yaml"];

export const isYamlFile = (file?: File | null) => {
  if (!file) return false;

  const hasValidMimeType =
    file.type && yamlMimeTypes.includes(file.type.toLowerCase());
  ...
  const isRunningOnWindows =
    file.type === "" && navigator.userAgent.includes("Windows");
