...
describe("removeEmpty", () => {
  ...
  describe("arrays", () => {
    it("should clean objects inside arrays", () => {
      ...
      expect(result).toEqual({
        items: [{ name: "Item1" }, { name: "Item2", data: "valid" }, undefined],
      });
    });
  ...
  describe("real world cases - publiccode.yml object", () => {
    const publiccodeData = {
      ...
      legal: {
        ...
        authorsFile:
          "https://github.com/italia/design-angular-kit/blob/main/AUTHORS",
      },
      maintenance: {
        type: "community",
        contacts: [
    ...
    it("should remove empty strings in contacts", () => {
      ...
      expect(result.maintenance.contacts[0]).toHaveProperty(
        "name",
        "Designers Italia",
      );
    ...
    it("should handle deep changes in nested object", () => {
      ...
      expect(result.maintenance.contacts?.[0]).not.toHaveProperty(
        "emptyContact",
      );
