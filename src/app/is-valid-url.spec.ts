...
fdescribe("isValidUrl", () => {
  test("valid URLs", () => {
    ...
    expect(
      isValidUrl("https://example.com:3000/path?query=value#section"),
    ).toBe(true);
  });
  ...
  test("valid URLs (must contain a path)", () => {
    ...
    expect(isValidUrl("https://example.com/path/to/resource", params)).toBe(
      true,
    );
    expect(isValidUrl("https://example.com/path?query=test", params)).toBe(
      true,
    );
  ...
  test("edge cases", () => {
    ...
    expect(
      isValidUrl("https://example.com/path/to/page?query=string#fragment"),
    ).toBe(true);
    expect(isValidUrl("https://example.com/path?valid=true&data[]=123")).toBe(
      true,
    );
  });
});
